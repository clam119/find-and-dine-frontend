...
export function ReviewsSummary(props: { reviews: any }) {
  ...
  const votes = Math.round(10*((reviewObj.reviewsDistribution.oneStar * 1) +
    (reviewObj.reviewsDistribution.twoStar * 2) +
    (reviewObj.reviewsDistribution.threeStar * 3) +
    (reviewObj.reviewsDistribution.fourStar * 4) +
    (reviewObj.reviewsDistribution.fiveStar * 5))/reviewObj.reviewsCount)/10
    ...
    return (
        ...
        <View style={{ width: '100%', height: '50%', justifyContent: 'space-around', padding:'5%'}}>
          ...
          <Text style={{ textAlign: "center", alignSelf: 'center', fontSize: 15, color:primary}}>{reviewObj.reviewsCount} reviews</Text>
        </View> 
